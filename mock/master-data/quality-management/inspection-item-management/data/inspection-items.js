...
const generateInspectionItemsData = () => {
  ...
  // 生成更多随机检验项目
  for (let i = 6; i <= 50; i++) {
    const method = Mock.mock(
      '@pick(["VISUAL", "MEASUREMENT", "TESTING", "SAMPLING", "CHEMICAL"])'
    );
    const product = Mock.mock(
      '@pick(["PRODUCT_A", "PRODUCT_B", "PRODUCT_C", "ALL"])'
    );
    const category = Mock.mock(
      '@pick(["Appearance", "Dimension", "Performance", "Chemical", "Physical", "Mechanical"])'
    );
    const dataType = Mock.mock(
      '@pick(["Numeric", "Text", "Boolean", "Enum", "Range"])'
    );
