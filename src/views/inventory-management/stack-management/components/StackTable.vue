    ...
    >
      <template #toolbar-left>
        ...
        <slot name="toolbar-left">
          <span v-if="selectedRows.length" class="stack-table__selection-indicator">
    ...
    >
      ...
      <template #stackCode="{ value }">
        <el-tooltip
          ...
          :content="copyTooltip"
        >
          <span class="stack-table__code" @click="handleCopyStackCode(value)">
            {{ value }}
      ...
      <template #currentLocation="{ row }">
        ...
        <span v-else class="stack-table__placeholder">-</span>
      </template>
...
import {
  STACK_TABLE_COLUMNS,
  STACK_DEFAULT_VISIBLE_COLUMNS,
  STACK_STATUS_TAG_CONFIG,
  STACK_TABLE_TOOLBAR_CONFIG
} from '../constants/table-config'
import { DEFAULT_SORT, STACK_STATUS } from '../constants'
...
export default {
  ...
  data() {
    return {
      columnSettingsKeyPrefix: 'stackManagementTableColumns',
      selectedRows: []
    }
  },
  computed: {
    // 列配置选项
    columnOptions() {
      return STACK_TABLE_COLUMNS
    },
    // 默认可见列
    defaultVisibleColumns() {
      return STACK_DEFAULT_VISIBLE_COLUMNS
    },
    // 料垛状态配置
    stackStatusConfig() {
      return STACK_STATUS_TAG_CONFIG
    },
    // 工具栏配置
    toolbarProps() {
      return {
  ...
  methods: {
    ...
    resolveRowClass({ status }) {
      ...
      const type = this.stackStatusConfig.typeMap[status]
      return type ? `stack-table__row--${type}` : ''
    },
    // 获取操作按钮配置
    getActionButtons(row) {
      ...
      // 只有ACTIVE状态的料垛才显示拆垛按钮
      if (row.status === STACK_STATUS.ACTIVE) {
        buttons.push({
          text: '拆垛',
          action: 'destack',
          icon: 'el-icon-delete',
...
function fallbackCopyText(text, messageInstance) {
  ...
  try {
    ...
    messageInstance?.success('料垛编号已复制')
  } catch (error) {
...
// 行状态样式
.stack-table__row--success {
  background-color: rgba(82, 196, 26, 0.08);
}

.stack-table__row--info {
