/**
 * 文件名称：StackSearch.vue
 * 文件描述：料垛搜索组件，基于全局SearchForm实现筛选功能
 * 创建日期：2025-01-10
 * 修改记录：
 *   - 2025-01-10: 初始创建，实现料垛搜索功能
 */
<template>
...
import { STACK_SEARCH_FORM_CONFIG, DEFAULT_PAGINATION, DEFAULT_SORT } from '../constants'
...
export default {
  ...
  computed: {
    orderedSearchItems() {
      ...
      const primary = STACK_SEARCH_FORM_CONFIG.filter((item) => item.priority === 'primary')
      const advanced = STACK_SEARCH_FORM_CONFIG.filter((item) => item.priority === 'advanced')
      ...
      Object.keys(values).forEach((key) => {
        ...
        if (key === 'createdAtRange') {
          if (Array.isArray(value) && value.length === 2) {
            // 转换为ISO 8601格式
            result.createdAtStart = new Date(value[0]).toISOString()
            result.createdAtEnd = new Date(value[1]).toISOString()
          }
          return
        }
