...
export default {
  ...
  computed: {
    ...
    errorMessages() {
      ...
      // 校验段序号的连续性（必须从1开始连续递增）
      const segmentOrderError = this.validateSegmentOrderContinuity()
  ...
  methods: {
     ...
     */
    validateSegmentOrderContinuity() {
      // 过滤出有段序号的风机（null 表示全局风机，不参与校验）
      const fansWithSegmentOrder = this.fans.filter(fan => fan.segmentOrder !== null && fan.segmentOrder !== undefined)
